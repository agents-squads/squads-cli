...
const tenant = program
  ...
  .addHelpText('after', `
Examples:
  $ squads tenant show acme-corp          Show tenant info
  $ squads tenant secrets list acme-corp  List configured secrets
  $ squads tenant secrets set acme-corp bigquery --project my-proj --service-account ./sa.json
  $ squads tenant verify acme-corp        Verify all secrets work
`)
  .action(() => {
    tenant.outputHelp();
  });
