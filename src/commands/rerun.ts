...
function getFailedRuns(prNumber: number, _repo: string): Run[] {
  try {
    ...
    const runs = JSON.parse(runsOutput || '[]') as Omit<Run, 'id'>[];
    return runs
      .filter(run => run.status === 'completed' && run.conclusion === 'failure')
      .map(run => ({ ...run, id: run.databaseId as number }));
  } catch {
    return [];
  }
}
...
export async function rerunCommand(options: {
  ...
  writeLine();
  writeLine(`  ${colors.cyan}${icons.info} PR #${prNumber}${RESET} ${colors.dim}(${repo})${RESET}`);
  writeLine();
  ...
  for (const run of failedRuns) {
    ...
    writeLine(`  ${colors.red}✗${RESET} ${bold(run.name)} ${colors.dim}(${run.event}, ${time})${RESET}`);
