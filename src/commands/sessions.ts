...
export async function sessionsSummaryCommand(
  ...
  if (data.squads.length > 0) {
    const w = { squad: 14, actions: 18, outputs: 44 };
    const tableWidth = w.squad + w.actions + w.outputs + 6;

    writeLine(`  ${colors.green}${icons.active}${RESET} ${bold}${data.squads.length} Squads Completed${RESET}`);
    writeLine();
    ...
    for (const squad of data.squads) {
      const row = `  ${colors.purple}${box.vertical}${RESET} ` +
        `${colors.cyan}${padEnd(squad.name, w.squad)}${RESET}` +
        `${padEnd(squad.actions, w.actions)}` +
        `${padEnd(squad.outputs, w.outputs)}` +
        `${colors.purple}${box.vertical}${RESET}`;
      writeLine(row);
    }
